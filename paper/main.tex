...
carry the content written back to the residual stream. Existing architectures and low-rank methods
nevertheless allocate them identical capacity. We ask whether selection is cheaper than content.
...
methods and experiment code, and \todo{if true: to help interpret results}. We have not used generative AI
tools to generate synthetic data sets, to formulate or prove mathematical claims, for translation, or for
qualitative data analysis, and \todo{list the remaining required-disclosure tasks that do not apply}.
...
parts of this paper, and proposing the title. We have reviewed all AI-assisted work:
\todo{describe truthfully, e.g. ``the authors ran every experiment themselves, checked the AI-written code with
unit tests (exact reconstruction at full rank, parameter-count checks, causality), verified every number in
the paper against the logged result files, read every cited paper, and checked AI-suggested ideas for prior
work through a manual literature search''}. We take responsibility for the final content of this work,
including text, claims or artifacts produced with the aid of generative AI.

\subsection*{Reproducibility statement}
\todo{Point to: Section~\ref{sec:method}; the appendix with all hyperparameters; the anonymized code in the
supplementary material (training, truncation, healing, benchmark and plotting scripts; every figure and table is
generated from logged JSON files); public models and datasets used.}
